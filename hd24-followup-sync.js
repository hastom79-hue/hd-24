...
function previewReady(){const p=$('hd24Preview');return !!(p&&p.style.display!=='none'&&$('hd24PreviewSubject')?.textContent)}
...
function trySync(reason){
  ...
  if(previewReady()){
    ...
    lastState='';
    logSafe('후속조치 패키지 확인: 기존 메일 Preview 사용');
    return;
  }
  ...
  try{
    ...
    setTimeout(()=>{
      if(previewReady()){
        downloadReplyOnce();
        completedSignature=sig;
        logSafe('후속조치 패키지 완료: 메일 Preview + 회신용 Excel 준비');
      }else{
...
function reset(reason){completedSignature='';runningSignature='';lastState='';[0,100,300,700,1500,3000,6000,10000,15000].forEach(ms=>setTimeout(()=>trySync(reason),ms))}
function wire(){
