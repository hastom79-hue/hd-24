...
(async()=>{
  ...
  let mailClicks=0,replyClicks=0,blobSeq=0,releaseLoad;const logs=[];
  ...
  class FakeWorkbook{constructor(){this.worksheets=[];this.xlsx={load:async()=>{await loadGate;this.worksheets=[ws]}}}}
  const sandbox={
  ...
  assert.equal(mailClicks,1,'in-flight cycle must create first Preview once');
  assert.equal(replyClicks,1,'in-flight cycle must request first reply workbook once');
  assert.equal(logs.filter(x=>x.includes('후속조치 패키지 완료:')).length,0,'validation is deliberately still in flight');

  // Hammer every wake-up path before ExcelJS validation resolves.
  for(let i=0;i<4;i++)sandbox.dispatchEvent(actionEvent());
  for(let i=0;i<4;i++)sandbox.dispatchEvent(new sandbox.CustomEvent('hd24-safe-reflect-complete',{detail:{signature:sig}}));
  for(let i=0;i<8;i++)for(const fn of intervals)fn();
  await new Promise(r=>setTimeout(r,5));
  assert.equal(mailClicks,1,'duplicate in-flight wakeups must not create another Preview');
  assert.equal(replyClicks,1,'duplicate in-flight wakeups must not create another reply workbook');

  releaseLoad();
  await new Promise(r=>setTimeout(r,20));
  assert.equal(mailClicks,1,'after validation resolves Preview must still be exactly once');
  assert.equal(replyClicks,1,'after validation resolves reply workbook must still be exactly once');
  assert.equal(logs.filter(x=>x.includes('후속조치 패키지 완료:')).length,1,'in-flight cycle must complete exactly once after validation resolves');

  console.log('PASS follow-up in-flight dedupe: duplicate action/safe/watchdog signals during async reply validation stay Preview=1, Reply=1, Complete=1');
