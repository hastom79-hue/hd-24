...
const mappings=[
  ...
  {masterRow:7,direction:'상향',kpiKr:'KPI C'}
];
...
assert.strictEqual(post.targetComparableForUnit('건/년'),false,'annual count target must not be compared to one month');
assert.strictEqual(post.targetComparableForUnit('%'),true,'percentage target should remain comparable');
