...
// It snapshots signature/cycle/plant/month/items before ExcelJS async work begins and
// drops the file completely if a new upload cycle becomes active before writeBuffer resolves.
...
function context(){
  ...
  if(!sig||!cy||!p||!preview||preview.style.display==='none'||preview.dataset.hd24Signature!==sig||Number(preview.dataset.hd24Cycle)!==cy)return null;
  ...
  return {sig,cycle:cy,plant:p,plantName:plantName(p),month:Number(snap.month),items,rows};
}
function stillCurrent(c){const preview=$('hd24Preview');return signature()===c.sig&&cycle()===c.cycle&&preview?.dataset.hd24Signature===c.sig&&Number(preview?.dataset.hd24Cycle)===c.cycle&&preview?.dataset.hd24Rows===c.rows}
async function buildAndDownload(c){
  ...
  if(!stillCurrent(c)){logSafe(`회신 Excel stale 생성 차단: cycle ${c.cycle} → current ${cycle()} / 새 업로드가 검증 중 시작됨`);return false}
