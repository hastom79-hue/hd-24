...
// same action-export snapshot and Preview metadata.
...
function previewReady(sig){const p=$('hd24Preview'),meta=previewMeta(sig);return !!(p&&p.dataset.hd24Signature===sig&&meta?.valid)}
function tagPreview(sig){
  const p=$('hd24Preview'),meta=previewMeta(sig),snap=snapshotFor(sig);
  if(!p||!meta?.valid||!snap)return false;
  p.dataset.hd24Signature=sig;p.dataset.hd24Month=String(snap.month);p.dataset.hd24Count=String(meta.count);return true;
}
function clearPreview(){
  const p=$('hd24Preview');if(p){p.style.display='none';delete p.dataset.hd24Signature;delete p.dataset.hd24Month;delete p.dataset.hd24Count;}
...
async function validateReplyFile(file,sig){
  const snap=snapshotFor(sig),meta=previewMeta(sig);if(!snap||!meta?.valid)throw new Error('Preview/분석스냅샷 상태 불일치');
  ...
  const lookup=snapshotLookup(snap),rows=[],seen=new Set();
  ws.eachRow((row,rn)=>{
    if(rn===1)return;
  ...
  for(const r of rows){
    ...
    const uniq=String(src.masterRow);if(seen.has(uniq))throw new Error(`회신 Excel 중복 KPI: ${r.kpiEn||r.kpiKr}`);seen.add(uniq);
  ...
  return {count:rows.length,month:Number(snap.month),plant:plantName(),fileName:file.name};
}
...
function requestReply(sig){
  if(!sig||replyDownloadedSignature===sig||replyRequestedSignature===sig||!snapshotFor(sig))return;
...
function completeIfReady(sig,verified){
  const meta=previewMeta(sig),snap=snapshotFor(sig);if(!verified||!meta?.valid||!snap||replyDownloadedSignature!==sig)return;
  completedSignature=sig;runningSignature='';lastState='';
  logSafe(`후속조치 패키지 완료: ${verified.plant} ${verified.month}월 Preview ${meta.count} KPI ↔ 회신 Excel ${verified.count}행 Target/Actual/Status 검증 PASS`);
}
  ...
  HTMLAnchorElement.prototype.click=function(){
    ...
    if(file&&sig&&name===file.name){
      ...
      Promise.resolve().then(()=>validateReplyFile(file,sig)).then(v=>{
        ...
        logSafe(`회신 Excel 실제파일 검증 PASS: ${v.fileName} / ${v.count}행`);completeIfReady(sig,v);
      }).catch(e=>{
...
function stateText(a,sig){return `safe=${window.hd24SafeReflectSuccessSignature===sig} / actionExport=${actionExportSignature===sig} / snapshot=${!!snapshotFor(sig)} / replyVerified=${replyDownloadedSignature===sig} / month=${a.month||'-'} / total=${a.total} / current=${a.current}`}
function trySync(reason){
  ...
  if(previewReady(sig)){
    requestReply(sig);
    if(replyDownloadedSignature===sig){completeIfReady(sig,{count:previewMeta(sig).count,month:snap.month,plant:plantName(),fileName:'verified'})}
  ...
  try{
    ...
    const verify=()=>{
      ...
      if(checks<8)setTimeout(verify,100);else{runningSignature='';logSafe('후속조치 패키지 재시도 대기: Preview 월/사업장/건수 검증 실패')}
    };
    setTimeout(verify,0);
...
function actionExportComplete(e){
  ...
  actionExportSignature=sig;logSafe(`후속조치 분석스냅샷 고정: ${month}월 ${items.length}건 / ${d.fileName||'분석후속조치본'}`);trySync('action export complete');
}
function wire(){
