...
function render(){
  ...
  body.innerHTML=rows.length?rows.map(r=>`<tr><td>${esc(plantLabel(r.plant))}</td><td>${esc(r.targetMonth||'-')}</td><td title="${esc(r.kpi)}"><b>${esc(r.kpiEn||r.kpi)}</b></td><td>${esc(dt(r.preparedAt))}</td><td>${r.sentAt?`<span class="pill ok">${esc(dt(r.sentAt))}</span>`:'-'}</td><td>${r.openedAt?esc(dt(r.openedAt)):'-'}</td><td>${r.replyAt?`<b>${esc(dt(r.replyAt))}</b>`:'-'}</td><td>${esc(r.replySequence||'-')}</td><td>${esc(r.responder||'-')}</td><td>${r.recurrence?`<span class="pill miss">${esc(r.recurrence)}</span>`:'-'}</td><td>${esc(r.reason||'-')}</td><td>${esc(r.rootCause||'-')}</td><td>${esc(r.plan||'-')}</td><td>${esc(r.owner||'-')}</td><td>${esc(r.due||'-')}</td><td>${esc(r.nextTarget||'-')}</td></tr>`).join(''):`<tr><td colspan="16" style="text-align:center;padding:18px;color:#66707d">조건에 맞는 메일/회신 이력이 없습니다.</td></tr>`;
}
