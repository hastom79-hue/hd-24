...
ok(src.includes('function currentCycle(){return Number(window.hd24ActionCycle)||0}'),'follow-up sync must bind to action upload cycle');
ok(src.includes('Number(s.cycle)!==Number(cycle)'),'snapshot must reject stale upload cycle');
ok(src.includes('p.dataset.hd24Cycle=String(cycle)'),'Preview cycle tag missing');
ok(src.includes("if(currentCycle()!==cycle||signature()!==sig)throw new Error('회신 Excel 검증 중 업로드 cycle 변경')"),'reply validation must fail closed across cycle change');
ok(src.includes('eventCycle!==cycle'),'action-export event cycle must match current cycle');
ok(src.includes('replyDownloadedCycle===cycle'),'reply completion must belong to current cycle');
...
ok(src.includes('p.dataset.hd24Rows=watchFingerprint(sig,cycle)'),'Preview exact-set fingerprint missing');
...
ok(src.includes('async function validateReplyFile(file,sig,cycle)'),'actual reply workbook verifier missing');
...
ok(action.includes('cycle:workCycle'),'action export must emit upload cycle');
ok(loader.includes('hd24-action-export.js?v=27'),'production loader must use action export v27');
ok(loader.includes('hd24-followup.js?v=24'),'production loader must use legacy UI module v24 with owner guard');
ok(loader.includes('hd24-followup-sync.js?v=8'),'production loader must use follow-up sync v8');
ok(refresh.includes('hd24-followup-sync.js?v=8'),'refresh helper must preload follow-up sync v8');

async function scenario({wrongSet=false,staleCycleEvent=false}){
  const listeners={};
  ...
  sandbox.window=sandbox;sandbox.hd24SafeReflectSuccessSignature=currentSig;sandbox.hd24ActionCycle=7;
  ...
  sandbox.dispatchEvent(new sandbox.CustomEvent('hd24-action-export-complete',{detail:{signature:currentSig,cycle:staleCycleEvent?6:7,month:7,items:snapshotItems,fileName:'master_분석후속조치본.xlsx'}}));
...
(async()=>{
  ...
  ok(good.sandbox.hd24FollowupSyncOwnsAutoPackage===true,'v8 ownership flag missing');
  ...
  ok(good.preview.dataset.hd24Signature===good.currentSig&&good.preview.dataset.hd24Cycle==='7'&&good.preview.dataset.hd24Month==='7'&&good.preview.dataset.hd24Count==='1','Preview identity/cycle/count tags mismatch');
  ...
  const stale=await scenario({staleCycleEvent:true});
  ok(stale.mailClicks===0&&stale.replyClicks===0,'stale same-signature action-export event from prior cycle must be ignored');
  ok(!stale.logs.some(x=>x.includes('후속조치 패키지 완료:')),'stale cycle must never complete follow-up package');

  console.log('PASS follow-up v8: exact watch set -> Preview/reply once; wrong set and stale same-signature cycle rejected fail-closed');
